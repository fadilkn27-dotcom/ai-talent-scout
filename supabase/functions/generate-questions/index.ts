...
serve(async (req) => {
  ...
  try {
    const { role, skills, difficulty } = await req.json();
...
You MUST respond by calling the "generate_questions" function. Do not respond with plain text.`;
    ...
    const userPrompt = `Generate assessment questions for a "${role}" position.
              ...
              parameters: {
                ...
                properties: {
                  codingQuestions: {
                    type: "array",
                    items: { type: "string" },
                    description: "3 coding challenge descriptions",
                  },
                  algorithmProblems: {
                    type: "array",
                    items: { type: "string" },
                    description: "2 algorithm problem descriptions",
                  },
                  mcqs: {
                    type: "array",
                    items: { type: "string" },
